  ...
  <aside class="md:sticky md:top-20">
    ...
    <nav>
      <ul class="flex flex-col gap-0.5">
        <?php
          $navItems = [
            '#section-infos'    => ['Mon profil',       'M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2 M12 3a4 4 0 1 0 0 8 4 4 0 0 0 0-8', false],
            '#section-securite' => ['Sécurité',         'M3 11h18v11a2 2 0 01-2 2H5a2 2 0 01-2-2V11z M7 11V7a5 5 0 0110 0v4', false],
            '#section-panier'   => ['Mon panier',       'M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z M3 6h18 M16 10a4 4 0 01-8 0', false],
            '#section-favoris'  => ['Mes favoris',      'M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l8.84 8.84 8.84-8.84a5.5 5.5 0 000-7.78z', false],
            '/logout'           => ['Se déconnecter',   'M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4 M16 17l5-5-5-5 M21 12H9', true],
          ];
          foreach ($navItems as $href => [$label, $svgPath, $isLogout]):
        ?>
          <li>
            <a href="<?= $href ?>" class="flex items-center gap-2.5 px-3 py-2.5 rounded-md text-[.70rem] tracking-[.12em] uppercase transition-colors duration-150 no-underline
              <?= $isLogout ? 'text-[#b5654a] hover:bg-[rgba(181,101,74,.08)]' : 'text-[var(--color-muted)] hover:bg-[var(--color-warm)] hover:text-[var(--color-text)]' ?>
              <?= !$isLogout && $href==='#section-infos' ? 'bg-[var(--color-dark)] text-[var(--color-cream)]' : '' ?>
              js-nav-link">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="opacity-50 flex-shrink-0">
                <?php foreach (explode(' ', $svgPath) as $d): ?>
                  <path d="<?= htmlspecialchars($d) ?>"/>
                <?php endforeach; ?>
              </svg>
              <?= htmlspecialchars($label) ?>
            </a>
          </li>
          <?php if ($href === '#section-favoris'): ?><li><div class="h-px bg-[var(--color-border)] my-2"></div></li><?php endif; ?>
        <?php endforeach; ?>
      </ul>
    </nav>
  </aside>
  ...
  <main class="min-w-0">
    ...
    <section class="mb-12" id="section-infos">
      ...
      <form id="form-profil" class="flex flex-col gap-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label for="prenom" class="form-label">Prénom</label><input type="text" id="prenom" name="prenom" class="form-input" value="<?= htmlspecialchars($currentUser['prenom']) ?>" required /></div>
          <div><label for="nom" class="form-label">Nom</label><input type="text" id="nom" name="nom" class="form-input" value="<?= htmlspecialchars($currentUser['nom']) ?>" required /></div>
        </div>
        <div><label for="email" class="form-label">Adresse e-mail</label><input type="email" id="email" name="email" class="form-input" value="<?= htmlspecialchars($currentUser['email']) ?>" required /></div>
        <div class="mt-2"><button type="submit" class="btn-dark">Enregistrer</button></div>
      </form>
    </section>
    ...
    <section class="mb-12" id="section-securite">
      ...
      <form id="form-password" class="flex flex-col gap-4">
        <div><label for="current_password" class="form-label">Mot de passe actuel</label><input type="password" id="current_password" name="current_password" class="form-input" required /></div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label for="new_password" class="form-label">Nouveau</label><input type="password" id="new_password" name="new_password" class="form-input" required minlength="8" /></div>
          <div><label for="confirm_password" class="form-label">Confirmer</label><input type="password" id="confirm_password" name="confirm_password" class="form-input" required minlength="8" /></div>
        </div>
        <div class="mt-2"><button type="submit" class="btn-dark">Mettre à jour</button></div>
      </form>
    </section>
    ...
    <section class="mb-12" id="section-panier">
      ...
      <div class="py-10 px-6 text-center border border-dashed border-[var(--color-border)] bg-[var(--color-warm)]">
        <svg class="w-9 h-9 mx-auto mb-3 opacity-20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/></svg>
    ...
    <section class="mb-12" id="section-favoris">
      ...
      <div class="py-10 px-6 text-center border border-dashed border-[var(--color-border)] bg-[var(--color-warm)]">
        <svg class="w-9 h-9 mx-auto mb-3 opacity-20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l8.84 8.84 8.84-8.84a5.5 5.5 0 000-7.78z"/></svg>
    ...
    <section id="section-danger">
      ...
      <div class="flex items-center justify-between gap-5 p-6 border border-[rgba(181,101,74,.25)]">
        ...
        <button id="btn-delete-account" class="flex-shrink-0 px-5 py-2.5 text-[.72rem] tracking-[.18em] uppercase cursor-pointer border border-[rgba(181,101,74,.4)] text-[#b5654a] bg-transparent hover:bg-[#b5654a] hover:text-white hover:border-[#b5654a] transition-colors duration-200">
...
<div id="modal-delete" class="hidden fixed inset-0 bg-black/50 z-50 items-center justify-center">
  <div class="bg-white p-8 max-w-sm w-[90%]">
    ...
    <div class="flex gap-3">
      <button id="btn-confirm-delete" class="flex-1 px-4 py-2.5 text-[.72rem] tracking-[.18em] uppercase cursor-pointer border border-[#b5654a] text-[#b5654a] bg-transparent hover:bg-[#b5654a] hover:text-white transition-colors duration-200">Supprimer</button>
      <button id="btn-cancel-delete"  class="flex-1 px-4 py-2.5 text-[.72rem] tracking-[.18em] uppercase cursor-pointer border border-[var(--color-border)] text-[var(--color-text)] bg-transparent hover:border-[var(--color-text)] transition-colors duration-200">Annuler</button>
    </div>
  </div>
</div>
...
document.getElementById('form-profil').addEventListener('submit', async function(e) {
  ...
  } catch { showFeedback(fb, 'Une erreur est survenue.', false); }
  btn.disabled = false;
});
...
document.getElementById('btn-delete-account').addEventListener('click', () => { modal.style.display = 'flex'; });
...
document.getElementById('btn-confirm-delete').addEventListener('click', async () => {
  ...
  if (!pass) { showFeedback(fb, 'Veuillez entrer votre mot de passe.', false); return; }
  btn.disabled = true;
  const body = new FormData(); body.append('password', pass);
  try {
