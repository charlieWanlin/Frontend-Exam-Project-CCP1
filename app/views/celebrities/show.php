...
<nav class="breadcrumb-bar" aria-label="Fil d'Ariane">
  ...
  <a href="/celebrities">Célébrités</a><span>/</span>
  <span class="current"><?= htmlspecialchars($celebrity['nom']) ?></span>
</nav>

<!-- ── Hero profil ── -->
<section class="flex items-stretch max-w-[1280px] mx-auto px-[5vw] min-h-[75vh] bg-[var(--color-cream)]" aria-label="Profil de <?= htmlspecialchars($celebrity['nom']) ?>">

  <!-- Photo portrait -->
  <div class="flex-shrink-0 relative overflow-hidden" style="width:clamp(220px,28vw,380px)">
    <?php if (!empty($celebrity['photo'])): ?>
      <img src="/assets/img/<?= htmlspecialchars($celebrity['photo']) ?>" alt="Photo de <?= htmlspecialchars($celebrity['nom']) ?>" class="absolute inset-0 w-full h-full object-cover object-top" loading="eager" />
    <?php else: ?>
  ...
  <div class="flex-1 flex flex-col justify-center py-14 pl-16">
    ...
    <div class="anim-fu-a mb-4">
      <span class="text-[.58rem] font-medium tracking-[.22em] uppercase text-[var(--color-gold)] border border-[var(--color-gold)] px-3 py-1">
        <?= htmlspecialchars(ucfirst($celebrity['categorie'] ?? 'Célébrité')) ?>
      </span>
    </div>
    ...
    <h1 class="anim-fu-b" style="font-family:var(--font-heading); font-size:clamp(2.4rem,5vw,4.8rem); font-weight:600; line-height:.92; letter-spacing:-.02em; color:var(--color-text); margin:0">
      <?= htmlspecialchars($celebrity['nom']) ?>
    </h1>
    ...
    <?php
      $descMap = [
        'cinéma'      => 'Acteur·ice incontournable, son style sur et hors écran définit les tendances.',
        'chant'       => 'Artiste dont les tenues scéniques et les looks street font la une des magazines.',
        'mode'        => 'Figure de style reconnue, chaque apparition est une leçon de goût.',
        'sport'       => 'Athlète au style aussi affûté que ses performances, entre sportswear et élégance.',
        'influenceur' => 'Créateur·ice de contenu dont l\'esthétique inspire des millions d\'abonnés.',
      ];
      $desc = $descMap[strtolower($celebrity['categorie'] ?? '')] ?? 'Une icône dont chaque look est une source d\'inspiration pour la mode contemporaine.';
    ?>
    ...
    <div class="anim-fu-c stat-row pt-6 border-t border-[var(--color-border)] mb-8">
      <div class="stat-pill"><span class="val"><?= number_format((int)$celebrity['nb_looks']) ?></span><span class="lbl">Looks</span></div>
      <div class="stat-sep" aria-hidden="true"></div>
      <div class="stat-pill">
        <span class="val"><?= (int)$celebrity['popularite'] ?><span style="font-size:.75rem">/100</span></span>
        <span class="lbl">Popularité</span>
      </div>
      <?php if (!empty($categories)): ?>
        <div class="stat-sep" aria-hidden="true"></div>
        <div class="stat-pill"><span class="val"><?= count($categories) ?></span><span class="lbl">Styles</span></div>
      <?php endif; ?>
    </div>
...
<section id="section-looks" aria-labelledby="looks-title" class="max-w-6xl mx-auto px-6 py-16">
  ...
  <header class="mb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
    <div>
      ...
      <h2 id="looks-title" style="font-family:var(--font-heading); font-size:clamp(1.6rem,3.5vw,2.4rem); font-weight:600; line-height:1; color:var(--color-text)">
        Les looks de <?= htmlspecialchars($celebrity['nom']) ?>
      </h2>
    </div>
    <p id="looks-count-label" class="text-[.75rem] text-[var(--color-muted)] shrink-0" aria-live="polite">
  ...
  <div class="flex items-center justify-between mb-8 gap-y-3 flex-wrap" role="toolbar" aria-label="Filtres des looks">
    <div class="flex gap-1.5 flex-wrap" role="group">
      <button data-look-filter="tous" class="filter-pill active" aria-pressed="true" type="button">Tous</button>
      <?php foreach ($categories as $cat): ?>
        <button data-look-filter="<?= htmlspecialchars($cat) ?>" class="filter-pill" aria-pressed="false" type="button">
...
<script>window.CELEBRITY_ID = <?= (int)$celebrity['id'] ?>;</script>
<script src="/js/celebrity-show.js" defer></script>
