...
<style>
  ...
  /* ══════════════════════════════════════════
     HERO — layout identique au HTML de référence
     Photo à gauche (portrait serré), infos à droite
  ══════════════════════════════════════════ */
  .celeb-hero {
    display: flex;
    align-items: stretch;
  ...
  /* ── Colonne photo ── */
  .celeb-hero__img-wrap {
    flex-shrink: 0;
    width: clamp(220px, 28vw, 380px); /* petit & proportionné */
    align-self: stretch;
    position: relative;
    overflow: hidden;
  }
  ...
  @media (min-width: 768px) and (max-width: 1024px) {
    .celeb-hero__info { padding: 2.5rem 0 2.5rem 2.5rem; }
    .celeb-hero__img-wrap { width: clamp(200px, 32vw, 300px); }
  }
...
<nav class="breadcrumb-bar" aria-label="Fil d'Ariane">
  ...
  <a href="/celebrities">Célébrités</a>
  <span aria-hidden="true">/</span>
...
<section
  class="celeb-hero"
  aria-label="Profil de <?= htmlspecialchars($celebrity['nom']) ?>"
  style="background: var(--color-cream)"
>

  <!-- Photo portrait -->
  <div class="celeb-hero__img-wrap">
    <?php if (!empty($celebrity['photo'])): ?>
      <img
        src="/assets/img/<?= htmlspecialchars($celebrity['photo']) ?>"
        alt="Photo de <?= htmlspecialchars($celebrity['nom']) ?>"
        class="celeb-hero__img"
        loading="eager"
      >
    <?php else: ?>
      <div class="celeb-hero__img-placeholder"></div>
    <?php endif; ?>
  </div>

  <!-- Infos -->
  <div class="celeb-hero__info">
    ...
    <?php
      ...
      $cat  = strtolower($celebrity['categorie'] ?? '');
      $desc = $descMap[$cat] ?? 'Une icône dont chaque look est une source d\'inspiration pour la mode contemporaine.';
    ?>
    <p
    ...
    >
      <div class="stat-pill">
        <span class="val"><?= number_format((int)$celebrity['nb_looks']) ?></span>
      ...
      <div class="stat-pill">
        <span class="val">
          <?= (int)$celebrity['popularite'] ?><span style="font-size:.75rem">/100</span>
        </span>
...
<section id="section-looks" aria-labelledby="looks-title" class="max-w-6xl mx-auto px-6 py-16">
  ...
  >
    <div class="flex gap-1.5 flex-wrap" role="group" aria-label="Filtrer par style">
      <button data-look-filter="tous" class="look-filter-btn active" aria-pressed="true" type="button">
        Tous
      </button>
      <?php foreach ($categories as $cat): ?>
        <button
          data-look-filter="<?= htmlspecialchars($cat) ?>"
...
<script>
  window.CELEBRITY_ID = <?= (int)$celebrity['id'] ?>;
</script>
